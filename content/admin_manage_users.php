...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        set_msg("Group '{$user_id}' has been added to {$group_id}", "success");
...
try {
    ...
    if ($result->rowCount() > 0) {
        ...
        echo "<tr><th>ID</th><th>User Name</th><th>Name</th></tr>";
        foreach ($result as $row) {
            $user_row = return_field_data($pdo, "users", "id", $row['user_id']);
            echo "<tr><td>{$user_row['id']}</td><td>{$user_row['username']}</td><td>{$user_row['lastname']},{$user_row['firstname']}</td></tr>";
        }
        echo "</table>";
    } else {
        echo "No users in the group table";
    }
} catch (PDOException $e) {
